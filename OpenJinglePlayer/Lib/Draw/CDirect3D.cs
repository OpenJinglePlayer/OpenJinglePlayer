...
namespace OpenJinglePlayer.Lib.Draw
{
    class CDirect3D : Form, IDraw
    {
        ...
        public CDirect3D()
        {
            ...
            _PresentParameters.BackBufferHeight = 800;
            _PresentParameters.BackBufferWidth = 600;
        ...
        public void EnterFullScreen()
        {
            ...
            this.SetDesktopLocation(Screen.AllScreens[ScreenNr].Bounds.Left, Screen.AllScreens[ScreenNr].Bounds.Top);
        ...
        public bool Init()
        {
            if (!_Device.Disposed)
            {
                this.Text = "OpenJinglePlayer Video Window";
                float width = ClientSize.Width;
                float height = ClientSize.Height;
                Matrix translate = Matrix.Translation(new Vector3(-width / 2, height / 2, 0));
                Matrix projection = Matrix.OrthoOffCenterLH(-width / 2, width / 2, -height / 2, height / 2, -100, 100);
                _VertexBuffer = new VertexBuffer(_Device, 10000 * (4 * Marshal.SizeOf(typeof(TexturedColoredVertex))), Usage.WriteOnly | Usage.Dynamic, VertexFormat.Position | VertexFormat.Texture1 | VertexFormat.Diffuse, Pool.Default);
        ...
        public void AfterDraw()
        {
            if (_Run)
            {
                ...
                //if (FullScreen && !_fullscreen) || (!FullScreen && _fullscreen))
                //{
                //    if (!_fullscreen)
                //        EnterFullScreen();
                //    else
        ...
        {
            DrawTexture(Texture, rect, color, new SRectF(0, 0, ClientSize.Width, ClientSize.Height, rect.Z), mirrored);
        }
        ...
        public void DrawTexture(STexture Texture, SRectF rect, SColorF color, SRectF bounds, bool mirrored)
        {
            if (_TextureExists(ref Texture))
            {
                ...
                if (!mirrored)
                {
                    ...
                    vert[0] = new TexturedColoredVertex(new Vector3(rx1, -ry1, rect.Z), new Vector2(x1, y1), c.ToArgb());
                    vert[1] = new TexturedColoredVertex(new Vector3(rx1, -ry2, rect.Z), new Vector2(x1, y2), c.ToArgb());
                    vert[2] = new TexturedColoredVertex(new Vector3(rx2, -ry2, rect.Z), new Vector2(x2, y2), c.ToArgb());
                    vert[3] = new TexturedColoredVertex(new Vector3(rx2, -ry1, rect.Z), new Vector2(x2, y1), c.ToArgb());
        ...
        public void DrawTextureReflection(STexture Texture, SRectF rect, SColorF color, SRectF bounds, float space, float height)
        {
            if (_TextureExists(ref Texture))
            {
                ...
                if (_TextureExists(ref Texture))
                {
                    ...
                    TexturedColoredVertex[] vert = new TexturedColoredVertex[4];
                    vert[0] = new TexturedColoredVertex(new Vector3(rx1, -ry1, rect.Z), new Vector2(x1, y2), c.ToArgb());
                    vert[1] = new TexturedColoredVertex(new Vector3(rx1, -ry2, rect.Z), new Vector2(x1, y1), transparent.ToArgb());
                    vert[2] = new TexturedColoredVertex(new Vector3(rx2, -ry2, rect.Z), new Vector2(x2, y1), transparent.ToArgb());
                    vert[3] = new TexturedColoredVertex(new Vector3(rx2, -ry1, rect.Z), new Vector2(x2, y2), c.ToArgb());
                    AddToVertexBuffer(vert, _D3DTextures[Texture.index], CalculateRotationMatrix(rect.Rotation, rx1, rx2, ry1, ry2));
                }
            }
        }
        ...
        {
            Matrix originTranslation = Matrix.Translation(new Vector3(-ClientSize.Width / 2, ClientSize.Height / 2, 0));
            if (rot != 0)
            {
