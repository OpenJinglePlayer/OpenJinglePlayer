...
using OpenJinglePlayer.Lib.Video.Acinerella;

namespace OpenJinglePlayer.Lib.Video
{
    ...
    class Decoder
    {
        ...
        private void DoOpen()
        {
            ...
            if (_Width < 1 || _Width > 2048 || _Height < 1 || _Height > 2048 || _VideoTimeBase < 0.01 || _VideoTimeBase > 0.2)
            {
                return;
            }
            ...
            lock (MutexFramebuffer)
            {
                ...
                if (num >= 0)
                {
                    if (frame.index == -1 || _Width != frame.width || _Height != frame.height)
                    {
                        ...
                        frame.data = new byte[_FrameBuffer[num].data.Length];
                        Array.Copy(_FrameBuffer[num].data, frame.data, frame.data.Length);
                        frame.NewImage = true;
                    }
                    else
                    {
